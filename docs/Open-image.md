...
If your raw images have one of these formats, start FishFeats "normally" by going to `Plugins>fishfeats>Start` and select your image.
...
Open the image with napari built-in readers or other plugin readers that are compatible with your image format. For example, this [plugin](https://github.com/AllenCellModeling/napari-aicsimageio#multi-scene-selection) offers options to open a lot of common microcospy data format.
...
This is necessary to correctly set the image name and path to save the results/configuration in the `results` folder in the same folder, or load previous results if there are any.
