...
Then, check that the 3D direction of the image is correct (the higher Z slices correspond to the apical cells = `top high z`).
...
When you reload this image in FishFeats, it will read this configuration file and reload it. 
...
You can then choose which steps to perform depending on what you need for your analysis by selecting the step in the `Main` panel in the right side of the window.

### A2 - Segment epithelial cells
...
Otherwise, select `do projection and segmentation`.

An interface opens to perform the projection.
You can either load a file of the projected junction staining if you have done it with another external software (e.g. CARE, LocalZProjector), by clicking on `choose file`. 

Otherwise, click on `Project now` to calculate the local projection with FishFeats.
...
Choose `Epyseg` in the `Method` interface to perform cell contour segmentation (or CellPose).
If you haven't installed `Epyseg` (not mandatory), you will get an error message. 
Install it in your virtual environment by typing `pip install epyseg`.
...
Wait while the program saves the data and replaces the projected cells in 3D.
...
If you open the result file (`AB3-HG-AQUCISITION-4CHANNELS-SHRNActrl-filtered_minicrop_results.csv`) in a table editor (e.g. Excel, R, Prism..) you can already analyse these data (number of cells, apical area, position, etc.).
This file will be enriched with all of the new information that you will add by performing other steps.

### A3 - Segment the smFish staining

Select <span style="background-color:#0b8300">Get RNA</span> in the `Main` interface for this step.

The interface that opens allows you to choose the channel to segment, and the parameters for the segmentation.
...
Choose the approximate size of one dot in XY and in Z (not the same as one another because with imaging, a circular spot is usually a rugby-balloon shape). 
...
If the automatic threshold does not yeild a good result, you can then unclick it and the value determined by Big-fish will be written in the parameter box.
...
You can change the points size with the interface in the right side, as well as color them by specific properties, in the `Point display` part in `EditRNA2` panel.

![RNA segmentation done](./imgs/step_rnagot.png)

### A4 - Assign the segmented dots to their cell and count them

`Automatic assignment` options allow to perform an automatic assignment of each spot to its most likely cell.
We will do assignment with the simplest method, which is the `Projection`.
Each spot will be assigned to the apical cell that it belongs to after an orthogonal Z-projection.

Select `Projection` in `Assignment method` and click on `Apply assignment`.

When the calculation is finished (0.004 min on our test image), the points are displayed with their assigned cell color. 
The contour of the cells are also overlaid to see the correspondance.

![RNA assigned](./imgs/step_rnaass.png)

You can correct the assignments with the available shortcuts (see text in the top-left part of the window) and with the `Point correction` panel.
...
e you are performing the correction by clicking on `Save RNAs` and click on `Save and quit RNA 2` when all is done.
...
If you have performed the steps for the previous example, the metadata are reloaded and already set.
Click on `Update` and directly go the next step.

Otherwise, follow the step [A1](#a1-open-the-image-and-checkset-the-metadata) of the previous example.
...
You can see the table of each cell and its value for the feature (or other features already classified) in the `Features table` menu. Click on `Update table` to update it to the latest value of the new feature.
